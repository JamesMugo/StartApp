 	...
 	if($output!=false){
 		...
 		while($rows= mysqli_fetch_assoc($output)){
 			...
 			if (empty($rows['profilePicture'])/*&& $rows['role_id']==2*/)
				...
				displayOneUser1($rows['userId'],$rows['firstName'],$rows['lastName'],$rows['country'],$interest,$userRow);
			}
				...
				else if(!empty($rows['profilePicture'])/*&& $rows['role_id']==3*/)
				{
					...
					$interest = getInterest($rows['interest_id']);
					displayOneUser2($rows['userId'],$rows['firstName'],$rows['lastName'],$rows['country'],$interest,$userRow);
 	...
 	if($output!=false){
        while($rows=mysqli_fetch_assoc($output)){
        	...
        	if(empty($rows['profilePicture'])){
        		//displaying user with pictures
					$interest = getInterest($rows['interest_id']);
        		displayBlocked1($rows['userId'],$rows['firstName'],$rows['lastName'],$rows['country'],$interest,$userRow);
        	}
        	else if(!empty($rows['profilePicture'])){
        		//displaying user with pictures
					$interest = getInterest($rows['interest_id']);
        		displayBlocked2($rows['userId'],$rows['firstName'],$rows['lastName'],$rows['country'],$interest,$userRow);
        	}
